...
class WC_Shippify_Admin_Back_Office {
	...
	public function shippify_admin_notices() {
		...
		// Empty Credentials
		if ( isset( $_GET['page'] ) && 'wc-settings' == $_GET['page'] && isset( $_GET['section'] ) && 'shippify-integration' == $_GET['section'] ) {
			return; // Don't show these notices in the same settings screen.
		}
		...
		if ( '' == $api_key || '' == $api_secret ) {
			...
			echo '<div class="updated fade"><p>' . sprintf( __( '%1$sWooCommerce Shippify is almost ready.%2$s To get started, %3$s connect your Shippify account%4$s.', 'woocommerce-shippify' ), '<strong>', '</strong>', '<a href="' . esc_url( $url ) . '">', '</a>' ) . '</p></div>' . "\n";
		}
	}
